...
function Section({ icon, title, count, children }) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider" style={MUTED}>
        {icon}
        {title}
        {count !== undefined && (
      ...
      <div role="dialog" aria-modal="true" aria-label={`Details for ${name}`}
        ...
        <div className="flex items-center justify-between px-5 py-4"
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[14px] font-medium truncate" style={{ color: '#ededed' }}>{name}</span>
            {data && (
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-5">
          {error && (
            <div className="px-3 py-2 rounded text-[12px] font-mono"
          ...
          {!data && !error && (
            <div className="text-[12px] font-mono py-8 text-center" style={MUTED}>Loading…</div>
          )}
          ...
          {data && (<>
            ...
            <Section icon={<Globe size={11} />} title="Ports" count={ports.length}>
              {ports.length === 0
                ? <span className="text-[12px] font-mono" style={MUTED}>none published</span>
                : ports.map((p, i) => <div key={i} className="text-[12px] font-mono" style={VALUE}>{p}</div>)}
            </Section>

            <Section icon={<HardDrive size={11} />} title="Bind mounts" count={(data.volumes || []).length}>
              {(data.volumes || []).length === 0
                ? <span className="text-[12px] font-mono" style={MUTED}>none captured</span>
                : data.volumes.map((v, i) => <div key={i} className="text-[12px] font-mono break-all" style={VALUE}>{v}</div>)}
            </Section>

            <Section icon={<KeyRound size={11} />} title="Environment" count={(data.environment_keys || []).length}>
              {(data.environment_keys || []).length === 0
                ? <span className="text-[12px] font-mono" style={MUTED}>none</span>
                : (
                  <div className="flex flex-wrap gap-1.5">
                    {data.environment_keys.map(k => (
                      <span key={k} className="text-[11px] font-mono px-1.5 py-0.5 rounded"
              ...
              <span className="text-[11px]" style={MUTED}>Values are hidden — they may contain secrets.</span>
            </Section>
                ...
                : (
                  <>
                    <button type="button" onClick={() => setShowLabels(s => !s)}
                      className="flex items-center gap-1 text-[12px] font-mono w-fit"
            ...
            <div className="px-3 py-2.5 rounded text-[11px] leading-relaxed"
