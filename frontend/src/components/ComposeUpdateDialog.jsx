    ...
    <div style={overlay} onClick={e => e.target === e.currentTarget && onCancel()}>
      <div style={panel} role="dialog" aria-modal="true" aria-label={`Compose update ${container.name}`}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #1a1a1a' }}>
          ...
          {diff && (
            <div style={{ fontSize: '13px', color: '#606060', ...mono }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {loading && (
            <div style={{ color: '#555', fontSize: '14px', textAlign: 'center', padding: '24px', ...mono }}>
              Checking for updates…
            </div>
          )}

          {error && (
            <div style={{ padding: '10px 12px', background: 'rgba(255,68,68,0.07)', border: '1px solid rgba(255,68,68,0.2)', borderRadius: '6px', color: '#ff4444', fontSize: '14px', ...mono }}>
              {error}
            </div>
          )}
          ...
          {diff && !loading && (<>
            ...
            <div style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid #1a1a1a', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ padding: '8px 12px', borderBottom: '1px solid #1a1a1a', fontSize: '12px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.1em', ...mono }}>
                Image change
              </div>
              <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                  <span style={{ fontSize: '12px', color: '#444', width: '52px', flexShrink: 0 }}>Current</span>
                  <code style={{ fontSize: '14px', color: '#ff6b6b', background: 'rgba(255,68,68,0.08)', padding: '2px 8px', borderRadius: '4px', wordBreak: 'break-all' }}>
                    {diff.current_image}
                  </code>
                </div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                  <span style={{ fontSize: '12px', color: '#444', width: '52px', flexShrink: 0 }}>Latest</span>
                  <code style={{ fontSize: '14px', color: '#50e3c2', background: 'rgba(80,227,194,0.08)', padding: '2px 8px', borderRadius: '4px', wordBreak: 'break-all' }}>
                    {diff.latest_image}
                  </code>
                </div>
                {!diff.has_change && (
                  <div style={{ fontSize: '13px', color: '#606060', marginTop: '4px', ...mono }}>
            ...
            <div style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid #1a1a1a', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ padding: '8px 12px', borderBottom: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.1em', ...mono }}>
                  {showFull ? 'compose file (editable)' : 'file changes'}
                </span>
                <button onClick={() => setShowFull(s => !s)}
                  style={{ fontSize: '12px', color: '#606060', background: 'none', border: '1px solid #222', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer', ...mono }}>
              ...
              {showFull ? (
                <textarea
                  ...
                  spellCheck={false}
                  style={{ width: '100%', minHeight: '220px', maxHeight: '320px', background: '#000', color: '#aaa', border: 'none', outline: 'none', ...mono, fontSize: '13px', lineHeight: 1.6, padding: '12px', resize: 'vertical', tabSize: 2 }}
                />
              ) : (
                diff.has_change
                  ? <DiffView current={diff.current_content} proposed={content} />
                  : <div style={{ padding: '16px 12px', fontSize: '13px', color: '#555', ...mono }}>No changes to the compose file.</div>
              )}
            </div>
